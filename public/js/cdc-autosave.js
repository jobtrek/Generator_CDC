(function () {
    ...
    let draftFormId       = draftInput.value ? parseInt(draftInput.value, 10) : null;
    let autosaveTimeout   = null;
    let isSaving          = false;
    let hasUnsavedChanges = false;

    function setIndicator(state, text) {
        indicator.className = `flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-medium transition-all duration-300 ${INDICATOR_STYLES[state]}`;
    ...
    function beaconSave() {
        if (!hasUnsavedChanges || !draftFormId) return;
    ...
    function scheduleAutosave() {
        ...
        autosaveTimeout = setTimeout(doAutosave, DEBOUNCE_DELAY);
    }
    ...
    setInterval(doAutosave, HEARTBEAT_INTERVAL);
})();
